...
export default function CanadaMarketPageContent() {
    ...
    <main className="pt-32 pb-24 px-6 overflow-x-hidden bg-[#020617] selection:bg-cyan-400 selection:text-black">
      <motion.div variants={container} initial="hidden" animate="show" className="max-w-7xl mx-auto">
        <section className="mb-32">
          <motion.h1 variants={item} className="text-sm font-mono tracking-[0.3em] text-cyan-400 uppercase mb-8">{`// REGIONAL DEPLOYMENT: CANADA`}</motion.h1>
          <motion.h2 variants={item} className="text-5xl md:text-8xl font-black tracking-tighter leading-[0.9] text-white mb-12 uppercase">
            NORTHERN SCALE.<br/>GLOBAL SPEED.
          </motion.h2>
          <motion.p variants={item} className="text-xl md:text-2xl font-light text-slate-400 max-w-2xl leading-relaxed">
            Engineering robust digital assets for Canada&apos;s growing tech hubs in Toronto, Vancouver, and Montreal.
          </motion.p>
        </section>
        ...
        <section className="mb-32 grid grid-cols-1 md:grid-cols-2 gap-12">
            <motion.div variants={item} className="p-10 border border-white/5 bg-white/[0.01]">
                ...
                <p className="text-slate-500 text-sm leading-relaxed mb-6">
                   Localized SEO and bilingual architectural support for the diverse Canadian market.
                </p>
                <div className="flex items-center gap-2 text-cyan-400 font-mono text-[10px]">
                   <ShieldCheck className="w-3 h-3" /> PIPEDA COMPLIANT STORAGE
                </div>
            </motion.div>
            <motion.div variants={item} className="p-10 border border-white/5 bg-white/[0.01]">
                <h4 className="text-2xl font-bold text-white mb-4 uppercase tracking-tighter">REGIONAL CASE STUDY</h4>
                <p className="text-slate-500 text-sm leading-relaxed mb-6">
                   Modernized a Toronto-based real estate portal, reducing property load times to under 300ms.
                </p>
                <div className="text-cyan-400 font-mono text-xs uppercase tracking-widest">&lt; 300ms LCP</div>
            </motion.div>
        </section>

        <section className="pt-32 pb-12 text-center">
           <Link href="/contact" className="px-12 py-5 bg-cyan-400 text-black font-mono font-black text-xl hover:bg-white transition-colors uppercase tracking-tighter">INITIATE_CANADA_DEPLOYMENT</Link>
        </section>
      </motion.div>
      <Footer suffix="CAN.MARKET.PROT" />
    </main>
  );
}
